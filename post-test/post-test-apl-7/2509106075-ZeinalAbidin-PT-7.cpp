...
#include <algorithm>
#include <stdexcept>
#if defined(_WIN32) && !defined(COMMON_LVB_UNDERSCORE)
...
bool login(Data &data, int percobaan = 3)
{
    ...
    {
        cout << termcolor::green << "Login Berhasil!\n"
             << termcolor::reset << endl;
        return true;
    }
    else
    {
        cout << termcolor::red << "Login Gagal. Sisa percobaan: " << percobaan - 1 << termcolor::reset << endl;
...
void tambah(int *pPanjang, Transaksi *arrTransaksi, int &tambahId)
{
    ...
    if (!(cin >> baru->jumlah_item))
    {
        bersihkanBuffer();
        throw invalid_argument("Jumlah item harus berupa angka!");
    }

    if (baru->jumlah_item < 1 || baru->jumlah_item > 5)
    {
        throw out_of_range("Jumlah item di luar batas logika (harus 1 - 5).");
    }
    ...
    for (int i = 0; i < baru->jumlah_item; i++)
    {
        ...
        cout << " Harga satuan: ";
        if (!(cin >> baru->detail_transaksi[i].harga) || baru->detail_transaksi[i].harga < 0)
        {
            bersihkanBuffer();
            throw invalid_argument("Harga harus berupa angka positif!");
        }

        cout << " Qty/Berat: ";
        if (!(cin >> baru->detail_transaksi[i].quantity) || baru->detail_transaksi[i].quantity <= 0)
        {
            bersihkanBuffer();
            throw invalid_argument("Kuantitas harus berupa angka lebih dari 0!");
        }
...
void ubah(int *pPanjang, Transaksi *arrTransaksi)
{
    ...
    {
        bersihkanBuffer();
        throw invalid_argument("Format ID salah! ID transaksi harus berupa angka.");
    }
    ...
    if (idx != -1)
    {
        ...
        if (!(cin >> ubahData->jumlah_item))
        {
            bersihkanBuffer();
            throw invalid_argument("Jumlah item harus berupa angka!");
        }

        if (ubahData->jumlah_item < 1 || ubahData->jumlah_item > 5)
        {
            throw out_of_range("Jumlah item di luar batas (harus 1 - 5).");
        }
        ...
        for (int j = 0; j < ubahData->jumlah_item; j++)
        {
            ...
            cout << "  Harga Satuan: ";
            if (!(cin >> ubahData->detail_transaksi[j].harga) || ubahData->detail_transaksi[j].harga < 0)
            {
                bersihkanBuffer();
                throw invalid_argument("Harga harus berupa angka positif!");
            }
            ...
            cout << "  Qty/Berat: ";
            if (!(cin >> ubahData->detail_transaksi[j].quantity) || ubahData->detail_transaksi[j].quantity <= 0)
            {
                bersihkanBuffer();
                throw invalid_argument("Kuantitas harus berupa angka lebih dari 0!");
            }
    ...
    {
        throw invalid_argument("Nomor data laundry tidak valid.");
    }
}
...
int main()
{
    ...
    do
    {
        try
        {
            ...
            {
                bersihkanBuffer();
                throw invalid_argument("Input menu tidak valid, masukkan angka 1-10.");
            }
        ...
        {
            cout << termcolor::red << termcolor::bold << "\n[ERROR] " << e.what() << termcolor::reset << "\nSilakan coba lagi.\n"
                 << endl;
        }
        catch (const out_of_range &e)
        {
            cout << termcolor::yellow << "\n[LOGIC ERROR] " << e.what() << termcolor::reset << "\nOperasi dibatalkan.\n"
                 << endl;
        }
        catch (const length_error &e)
        {
            cout << termcolor::magenta << "\n[SYSTEM WARNING] " << e.what() << termcolor::reset << endl;
        }
        catch (const exception &e)
        {
            cout << termcolor::grey << "\n[FATAL ERROR] Kesalahan sistem yang tidak terduga: " << e.what() << termcolor::reset << endl;
        }
    } while (pilihan != 10);
    return 0;
}